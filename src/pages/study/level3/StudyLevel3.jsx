...
function StudyPage(){
   ...
   const handleUserSubmit = async () => {
    ...
    const handleFeedback=async()=>{
                try{
                    const res=await fetch("http://3.34.150.31:8080/api/study/feedback",{
                        method:"POST",
                        headers:{
   ...
   const handleNext=async()=>{
    if (currentIndex<sentences.length-1){
        ...
        try{
            const response=await fetch(`http://3.34.150.31:8080/api/question/saveAll?chapterId=${chapterData?.chapterId}`,{
                method:'POST',
                credentials:'include',
            });
   ...
   async function saveFeedbacks(chapterId){
    const response=await fetch(`http://3.34.150.31:8080/api/study/feedback/saveAll?chapterId=${chapterId}`,{
        method:"POST",
        credentials:"include"
    });
    if (!response.ok){
