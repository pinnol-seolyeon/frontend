...
export async function fetchStudyStats() {
  try {
    const res = await fetch('https://finnol.site:8080/api/study/stats', {
      method: 'GET',
      credentials: 'include',
      headers: {
...
export async function fetchStudyTimeStats() {
  try {
    const res = await fetch('https://finnol.site:8080/api/study/preferred-time', {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
...
export async function fetchRadarScore() {
  const res = await fetch("https://finnol.site:8080/api/analysis/radar-score/compare", {
    credentials: "include"
  });
...
export async function fetchQuestionDates() {
  const res = await fetch('https://finnol.site:8080/api/study/questions/dates', {
    credentials: 'include',
  });
...
export async function fetchQuestionsByDate(dateStr) {
  const res = await fetch(`https://finnol.site:8080/api/study/questions/history?date=${dateStr}`, {
    credentials: 'include',
  });
